...
type RegisterStep = 'email' | 'password' | 'info' | 'webcam';
...
const RegisterForm = ({ onRegisterSuccess }: RegisterFormProps) => {
  ...
  const handleEmailCheck = async (e: FormEvent) => {
    ...
    try {
      const responseCheckMail = await checkEmailExists(email);

      if (responseCheckMail.exists === true && responseCheckMail.success === true) {
        setError(responseCheckMail.message);
        return;
      } else if (responseCheckMail.success === false) {
        setError(responseCheckMail.message);
        return
      }
  ...
  const handleInfoSubmit = (e: FormEvent) => {
    ...
    console.log("Informations personnelles validées");
  ...
  const handleWebcamSubmit = (e: FormEvent) => {
    e.preventDefault();
    stopWebcam();
    handleRegister();
  };
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <AnimatePresence mode="wait">
            ...
            <form onSubmit={handleEmailCheck}>
              ...
              <div className="flex justify-between">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`py-3 px-6 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition duration-200 flex items-center cursor-pointer ${loading ? "opacity-70 cursor-not-allowed" : ""
                    }`}
                >
                  {loading ? (
                    <>
            ...
            <form onSubmit={handleInfoSubmit}>
              ...
              <div className="mb-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div
                    className={`flex items-center justify-center py-2 px-4 border ${gender === 'male'
                  ...
                  <div
                    className={`flex items-center justify-center py-2 px-4 border ${gender === 'female'
                        ? 'bg-blue-50 border-blue-500 text-blue-600'
                        : 'border-gray-200 text-gray-700 hover:bg-gray-50'
                      } rounded-lg cursor-pointer transition-colors`}
                  ...
                  <div
                    className={`flex items-center justify-center py-2 px-4 border ${gender === 'other'
                        ? 'bg-blue-50 border-blue-500 text-blue-600'
                        : 'border-gray-200 text-gray-700 hover:bg-gray-50'
                      } rounded-lg cursor-pointer transition-colors`}
        ...
        {/* step 4: Webcam */}
        {currentStep === 'webcam' && (
          <FadeIn key="webcam-step" className="w-full">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Vérification Webcam (optionnel)</h2>

            <form onSubmit={handleWebcamSubmit}>
              <div className="mb-6">
                <div className="aspect-square relative rounded-xl overflow-hidden border border-gray-200 bg-gray-100">
                  {webcamError ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                      <FiAlertCircle className="text-red-500 text-4xl mb-3" />
                      <p className="text-gray-800 font-medium mb-2">{webcamError}</p>

                      {webcamPermissionDenied ? (
                        <p className="text-sm text-gray-500">
                          Vous avez refusé l'accès à votre caméra. Pour continuer, veuillez autoriser l'accès dans les paramètres de votre navigateur.
                        </p>
                      ) : (
                        <button
                          type="button"
                    ...
                    <div className="relative h-full">
                      ...
                      <div className="absolute bottom-4 left-0 right-0 flex justify-center space-x-2">
                        <button
                          type="button"
                          onClick={resetPhoto}
                          className="bg-white text-gray-800 py-2 px-4 rounded-lg shadow hover:bg-gray-100 transition-colors flex items-center"
                        >
                          <FiX className="mr-1" />
                          Reprendre
                        </button>
                ...
                <p className="mt-2 text-xs text-gray-500">
                  Cette étape est facultative. Si vous ne souhaitez pas allumer votre webcam, vous pouvez directement passer à l'inscription.
                </p>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`py-3 px-6 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition duration-200 flex items-center ${loading ? "opacity-70 cursor-not-allowed" : ""
                    }`}
                >
                  {loading ? (
                    <>
